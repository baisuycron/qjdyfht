...
const ordersSeed = [
  { id: "2026081109001001", product: "小儿氨酚烷胺颗粒/ZQ/A", spec: "6g*15袋", image: "/assets/trade-product-067053.jpg", price: 20, total: 18, promotionDiscounts: [{ name: "限时9折", amount: 2 }], time: "2026-08-11 09:36:18", store: "千金大药房株洲演示店", buyer: "演示会员A", account: "demo_user_01", receiver: "张*", phone: "188****1024", address: "湖南省 长沙市 演示地址***", delivery: "快递配送", status: "待付款", note: "—" },
  { id: "2026081109001002", product: "可孚医用棉签/YP/A", spec: "50支/袋", image: "/assets/trade-product-special.png", price: 3.5, total: 3.16, promotionDiscounts: [{ name: "会员专享折扣", amount: 0.34 }], time: "2026-08-11 09:28:45", store: "千金大药房车站北路演示店", buyer: "演示会员B", account: "demo_user_02", receiver: "李*", phone: "177****3256", address: "湖南省 株洲市 演示地址***", delivery: "同城配送", status: "待发货", note: "优先发货" },
...
function getOrderPricing(order) {
  ...
  const subtotal = Number(order.subtotal ?? productTotal);
  const freight = Number(order.freight ?? 0);
  const couponDiscount = Number(order.couponDiscount ?? 0);
  const promotionDiscounts = (order.promotionDiscounts || []).map(item => ({ ...item, amount: Number(item.amount) })).filter(item => item.amount > 0);
  const promotionDetailTotal = promotionDiscounts.reduce((sum, item) => sum + item.amount, 0);
  const discountTotal = Number((order.discountTotal ?? (promotionDiscounts.length ? promotionDetailTotal : productTotal + freight - couponDiscount - Number(order.total))).toFixed(2));
  const orderTotal = Number((productTotal + freight - discountTotal - couponDiscount).toFixed(2));
  return { quantity, productTotal, subtotal, freight, discountTotal, couponDiscount, orderTotal, promotionDiscounts };
}

function OrderDetail({ order, onBack, showToast }) {
  const [addressExpanded, setAddressExpanded] = useState(false);
  const [discountExpanded, setDiscountExpanded] = useState(Boolean(order.promotionDiscounts?.length));
  ...
  return <><div className="detail-toolbar"><button className="trade-secondary" onClick={onBack}>返回订单列表</button></div>
    ...
    <section className="trade-detail-card"><h2>订单清单</h2><table className="trade-table detail-product-table"><thead><tr><th>商品名称</th><th>商品编码</th><th>商品类型</th><th>单价/数量</th><th>小计</th></tr></thead><tbody><tr><td><div className="trade-product"><img src={order.image} alt="" /><div><span>{order.product}</span><small>{order.spec}</small></div></div></td><td>DEMO067053</td><td>非处方药</td><td>{formatMoney(order.price)} * {pricing.quantity}件</td><td className="price-red">¥ {formatMoney(pricing.subtotal)}</td></tr></tbody></table><div className="order-summary"><span>买家留言　无</span><dl><div><dt>商品总价</dt><dd>¥ {formatMoney(pricing.productTotal)}</dd></div><div className="discount-total-row"><dt>优惠合计</dt><dd className="discount-value">- ¥ {formatMoney(pricing.discountTotal)}</dd>{pricing.promotionDiscounts.length > 0 && <button className={discountExpanded ? "discount-expand-button expanded" : "discount-expand-button"} type="button" aria-label={discountExpanded ? "收起促销优惠明细" : "展开促销优惠明细"} aria-expanded={discountExpanded} onClick={() => setDiscountExpanded(!discountExpanded)}><FiChevronDown /></button>}</div>{discountExpanded && pricing.promotionDiscounts.map(item => <div className="promotion-discount-row" key={item.name}><dt>{item.name}</dt><dd>- ¥ {formatMoney(item.amount)}</dd></div>)}<div><dt>优惠券</dt><dd className="discount-value">- ¥ {formatMoney(pricing.couponDiscount)}</dd></div><div><dt>运费</dt><dd>¥ {formatMoney(pricing.freight)}</dd></div><div><dt>订单总额</dt><dd>¥ {formatMoney(pricing.orderTotal)}</dd></div><div className="paid"><dt>订单实付</dt><dd>¥ {formatMoney(pricing.orderTotal)}</dd></div></dl></div></section>
