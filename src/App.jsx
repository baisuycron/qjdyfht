...
function CombinationDateTimePicker({ id, label, value, opposite, mode, invalid, onChange, readOnly = false }) {
  ...
  const currentBusinessTime = formatBusinessDateTime(); const currentDate = currentBusinessTime.slice(0, 10); const currentTime = currentBusinessTime.slice(11, 16);
  const endDateFloor = mode === "end" ? [dateBound, currentDate].filter(Boolean).sort().at(-1) : "";
  ...
  const isDisabled = date => mode === "start" ? Boolean(dateBound && dateKey(date) > dateBound) : Boolean(endDateFloor && dateKey(date) < endDateFloor);
...
const combinationActivityProduct = (wareid, wareqty = 1) => ({ ...combinationProductBases.find(product => product.wareid === wareid), wareqty });
const combinationPriceActivities = [
  { wareid: 84791, warecode: "0084791", warename: "日常清洁护理组合", starttime: "2026-08-01 00:00:00", endtime: "2026-08-31 23:59:59", combinationPrice: 29.8, marketingType: "组合价", description: "日常清洁与咽喉护理用品组合。", cnt: 3, itemList: [combinationActivityProduct(6947), combinationActivityProduct(37692), combinationActivityProduct(37848)] },
  { wareid: 84792, warecode: "0084792", warename: "家庭常备护理组合", starttime: "2026-08-10 00:00:00", endtime: "2026-09-09 23:59:59", combinationPrice: 89.9, marketingType: "组合价", description: "家庭常备滋补与护理商品组合。", cnt: 2, itemList: [combinationActivityProduct(8520), combinationActivityProduct(25216)] },
  { wareid: 85120, warecode: "0085120", warename: "夏日防暑护理组合", starttime: "2026-07-15 00:00:00", endtime: "2026-08-25 23:59:59", combinationPrice: 29.9, marketingType: "组合价", description: "适用于夏季防暑与咽喉日常护理。", cnt: 2, itemList: [combinationActivityProduct(100212), combinationActivityProduct(37848, 2)] },
  { wareid: 85124, warecode: "0085124", warename: "儿童出行护理组合", starttime: "2026-09-01 00:00:00", endtime: "2026-09-30 23:59:59", combinationPrice: 29.8, marketingType: "组合价", description: "儿童出行常备用品组合。", cnt: 3, itemList: [combinationActivityProduct(100315), combinationActivityProduct(100316), combinationActivityProduct(100319)] },
  { wareid: 85125, warecode: "0085125", warename: "春季营养关爱组合", starttime: "2026-03-01 00:00:00", endtime: "2026-03-31 23:59:59", combinationPrice: 79.9, marketingType: "组合价", description: "已结束活动样本，用于查看历史组合购配置。", cnt: 2, itemList: [combinationActivityProduct(48488), combinationActivityProduct(23001, 2)] },
];
...
function CombinationPriceList({ setView, activities = combinationPriceActivities, onTerminate }) {
  ...
  const now = useCurrentTime();
  const [page, setPage] = useState(1); const [pageSize, setPageSize] = useState(10); const [jumpPage, setJumpPage] = useState("1");
  const [terminateTarget, setTerminateTarget] = useState(null); const [toastMessage, setToastMessage] = useState("");
...
function CombinationPriceForm({ setView, onSave, existingActivities = [], activity = null, mode = "create" }) {
  ...
  const initialProducts = (activity?.itemList || []).map(item => ({ ...(combinationProductCatalog.find(product => product.wareid === item.wareid) || {}), ...item }));
  const [title, setTitle] = useState(activity?.warename || ""); const [starttime, setStarttime] = useState(activity?.starttime || ""); const [endtime, setEndtime] = useState(activity?.endtime || ""); const [combinationPrice, setCombinationPrice] = useState(activity?.combinationPrice == null ? "" : String(activity.combinationPrice)); const [selectedProducts, setSelectedProducts] = useState(initialProducts); const [description, setDescription] = useState(activity?.description || ""); const [pickerOpen, setPickerOpen] = useState(false); const [message, setMessage] = useState(""); const [validationAttempted, setValidationAttempted] = useState(false); const [formToastMessage, setFormToastMessage] = useState("");
  const blockedProductReasons = useMemo(() => {
    const reasons = {};
    existingActivities.filter(item => item.wareid !== activity?.wareid && getActivityStatus(item) !== "已结束").forEach(item => (item.itemList || []).forEach(product => { reasons[product.wareid] = "已参与其他组合购"; }));
  ...
  const save = () => {
    ...
    if (parseBusinessDateTime(endtime) <= Date.now()) { setMessage(""); showFormToast("结束时间必须晚于当前时间"); return; }
    ...
    const price = Number(combinationPrice); const originalTotal = selectedProducts.reduce((total, product) => total + Number(product.price || 0) * Number(product.wareqty || 1), 0);
    if (price <= 0) { showFormToast("组合价格必须大于0"); return; }
    if (price >= originalTotal) { showFormToast("组合价格必须低于商品原价合计"); return; }
    onSave({ ...activity, wareid: activity?.wareid ?? Date.now(), warecode: activity?.warecode || nextCombinationActivityNo(existingActivities), warename: title.trim(), starttime, endtime, combinationPrice: price, marketingType: "组合价", cnt: selectedProducts.length, itemList: selectedProducts.map(item => ({ ...item, wareqty: item.wareqty ?? 1 })), description });
  };
  ...
  return <PageFrame crumb={crumb} section="marketing" setView={setView}>
    <section className={readOnly ? "panel combination-create-panel combination-view-panel" : "panel combination-create-panel"}><div className="combination-create-form">
      ...
      <div className="combination-form-row"><label className={requiredClass} htmlFor="combination-start">开始时间：</label><div className="combination-field-control"><CombinationDateTimePicker id="combination-start" label="开始时间" value={starttime} opposite={endtime} mode="start" invalid={startInvalid} readOnly={readOnly} onChange={value => { setStarttime(value); setMessage(""); }} />{startInvalid && <span className="combination-field-error">请选择开始时间</span>}</div></div>
      <div className="combination-form-row"><label className={requiredClass} htmlFor="combination-end">结束时间：</label><div className="combination-field-control"><CombinationDateTimePicker id="combination-end" label="结束时间" value={endtime} opposite={starttime} mode="end" invalid={endInvalid} readOnly={readOnly} onChange={value => { setEndtime(value); setMessage(""); }} />{endInvalid && <span className="combination-field-error">请选择结束时间</span>}</div></div>
      <div className="combination-form-row"><label className={requiredClass} htmlFor="combination-price">组合价格：</label><div className="combination-field-control"><input id="combination-price" className="combination-price-input" type="number" min="0" step="0.01" inputMode="decimal" readOnly={readOnly} aria-required={!readOnly} aria-invalid={priceInvalid} value={combinationPrice} onChange={readOnly ? undefined : event => { setCombinationPrice(event.target.value); setMessage(""); }} placeholder="请输入组合价格" />{priceInvalid && <span className="combination-field-error">请输入组合价格</span>}</div></div>
      <div className="combination-form-row product-row"><label className={requiredClass}>组合商品：</label><div className="combination-products-field">{!readOnly && <button className="secondary product-picker-btn" onClick={() => setPickerOpen(true)}>添加组合商品</button>}{selectedProducts.length > 0 ? <SelectedCombinationProducts products={selectedProducts} onUpdate={updateProduct} onRemove={removeProduct} readOnly={readOnly} /> : <span className="combination-view-empty">暂无组合商品</span>}{!readOnly && <p>组合商品最多可添加9个</p>}</div></div>
      <div className="combination-form-row description-row"><label htmlFor="combination-description">组合描述：</label><div><textarea id="combination-description" maxLength={500} value={description} readOnly={readOnly} onChange={readOnly ? undefined : event => setDescription(event.target.value)} /><span>{description.length}/500</span></div></div>
    ...
    </div></section>{formToastMessage && <div className="combination-form-toast" role="alert"><span aria-hidden="true">×</span>{formToastMessage}</div>}{pickerOpen && <CombinationProductPicker selectedProducts={selectedProducts} blockedProductReasons={blockedProductReasons} onConfirm={addProducts} onClose={() => setPickerOpen(false)} />}
  </PageFrame>;
}
...
export function App() {
  ...
  const terminateCombinationActivity = wareid => setCombinationActivities(current => current.map(activity => {
