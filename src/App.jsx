...
function CombinationPriceList({ setView, activities = combinationPriceActivities, onTerminate }) {
  const [activityName, setActivityName] = useState(""); const [activityStatus, setActivityStatus] = useState(""); const [startDate, setStartDate] = useState(""); const [endDate, setEndDate] = useState(""); const [productName, setProductName] = useState(""); const [productCode, setProductCode] = useState("");
  const [query, setQuery] = useState({ activityName: "", activityStatus: "", startDate: "", endDate: "", productName: "", productCode: "" });
  const now = useCurrentTime();
  const [page, setPage] = useState(1); const [pageSize, setPageSize] = useState(10); const [jumpPage, setJumpPage] = useState("1");
  ...
  const switchPage = target => { const next = Math.min(Math.max(target, 1), totalPages); setPage(next); setJumpPage(String(next)); };
  const runQuery = () => { setQuery({ activityName, activityStatus, startDate, endDate, productName, productCode }); switchPage(1); };
  const reset = () => { setActivityName(""); setActivityStatus(""); setStartDate(""); setEndDate(""); setProductName(""); setProductCode(""); setQuery({ activityName: "", activityStatus: "", startDate: "", endDate: "", productName: "", productCode: "" }); switchPage(1); };
