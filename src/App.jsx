...
function CombinationProductPicker({ selectedProducts, onConfirm, onClose }) {
  const [category, setCategory] = useState(""); const [keyword, setKeyword] = useState(""); const [query, setQuery] = useState({ category: "", keyword: "" }); const [pendingIds, setPendingIds] = useState([]); const [page, setPage] = useState(1); const [pageSize, setPageSize] = useState(10); const [jumpPage, setJumpPage] = useState("1");
  ...
  const reset = () => { setCategory(""); setKeyword(""); setQuery({ category: "", keyword: "" }); switchPage(1); };
  ...
  const selectableRows = rows.filter(product => !selectedProducts.some(item => item.wareid === product.wareid)); const selectedPageCount = selectableRows.filter(product => pendingIds.includes(product.wareid)).length; const allPageSelected = selectableRows.length > 0 && selectedPageCount === selectableRows.length; const somePageSelected = selectedPageCount > 0 && !allPageSelected;
  const toggleProduct = product => setPendingIds(current => current.includes(product.wareid) ? current.filter(id => id !== product.wareid) : [...current, product.wareid]);
  const togglePage = () => { const pageIds = selectableRows.map(product => product.wareid); setPendingIds(current => allPageSelected ? current.filter(id => !pageIds.includes(id)) : [...new Set([...current, ...pageIds])]); };
  const pendingProducts = combinationProductCatalog.filter(product => pendingIds.includes(product.wareid)); const remainingCount = Math.max(0, 9 - selectedProducts.length); const overLimit = pendingProducts.length > remainingCount;
  return <div className="modal-backdrop product-select-backdrop"><section className="modal combination-product-modal" aria-label="商品选择"><header><b>商品选择</b><button aria-label="关闭商品选择" onClick={onClose}><FiX /></button></header><div className="product-select-filters"><select aria-label="商品分类" value={category} onChange={event => setCategory(event.target.value)}><option value="">请选择商品分类（一级 / 二级）</option>{[...new Set(combinationProductCatalog.map(product => product.category))].map(option => <option key={option}>{option}</option>)}</select><input aria-label="商品名称或商品编码" value={keyword} onChange={event => setKeyword(event.target.value)} onKeyDown={event => event.key === "Enter" && runSearch()} placeholder="请输入商品名称/商品编码" /><button className="primary" onClick={runSearch}><FiSearch /> 搜索</button><button className="secondary" onClick={reset}>重置</button></div><div className="product-select-table-wrap"><table className="product-select-table"><thead><tr><th><label className="page-select-all"><input type="checkbox" aria-label="本页全选" checked={allPageSelected} ref={node => { if (node) node.indeterminate = somePageSelected; }} disabled={selectableRows.length === 0} onChange={togglePage} /><span>选择</span></label></th><th>商品名称</th><th>库存</th><th>售价</th></tr></thead><tbody>{rows.map(product => { const added = selectedProducts.some(item => item.wareid === product.wareid); const checked = pendingIds.includes(product.wareid); return <tr key={product.wareid} className={checked ? "selected" : ""}><td><div className="product-code-cell"><input type="checkbox" aria-label={`选择${product.warename}`} checked={checked} disabled={added} onChange={() => toggleProduct(product)} /><span>{product.warecode}</span></div></td><td><div className="product-name-cell"><img src={product.image} alt="" /><span>{product.warename}</span>{added && <em>已添加</em>}</div></td><td>{product.stock}</td><td>{product.price.toFixed(product.price % 1 === 0 ? 1 : 2)}</td></tr>; })}</tbody></table>{rows.length === 0 && <p className="product-select-empty">暂无符合条件的商品</p>}</div><div className="product-select-pagination"><span>共 {filteredProducts.length} 条</span><select aria-label="商品每页条数" value={pageSize} onChange={event => { setPageSize(Number(event.target.value)); setPage(1); setJumpPage("1"); }}><option value="10">10条/页</option><option value="20">20条/页</option><option value="50">50条/页</option></select><button aria-label="商品上一页" disabled={currentPage === 1} onClick={() => switchPage(currentPage - 1)}><FiChevronLeft /></button>{visiblePages.map((pageNo, index) => typeof pageNo === "number" ? pageNo === currentPage ? <b key={pageNo}>{pageNo}</b> : <button key={pageNo} onClick={() => switchPage(pageNo)}>{pageNo}</button> : <span key={`${pageNo}-${index}`}>•••</span>)}<button aria-label="商品下一页" disabled={currentPage === totalPages} onClick={() => switchPage(currentPage + 1)}><FiChevronRight /></button><span>前往</span><input aria-label="商品跳转页码" value={jumpPage} onChange={event => setJumpPage(event.target.value.replace(/\D/g, ""))} onKeyDown={event => event.key === "Enter" && switchPage(Number(jumpPage) || 1)} onBlur={() => switchPage(Number(jumpPage) || 1)} /><span>页</span></div><footer><span className={overLimit ? "product-select-summary error" : "product-select-summary"}>{overLimit ? `组合商品最多可添加9个，请取消${pendingProducts.length - remainingCount}个后再确认` : `已选择 ${pendingProducts.length} 个，还可选择 ${remainingCount - pendingProducts.length} 个`}</span><div className="product-select-footer-actions"><button className="secondary" onClick={onClose}>取消</button><button className="primary" disabled={pendingProducts.length === 0 || overLimit} onClick={() => onConfirm(pendingProducts)}>确认</button></div></footer></section></div>;
}

function SelectedCombinationProducts({ products, onUpdate, onRemove }) {
  return <div className="combination-selected-table-wrap"><table className="combination-selected-table"><thead><tr>{["商品图片", "商品编码", "商品名称", "参考价(元)", "商品规格", "数量", "操作"].map(title => <th key={title}>{title}</th>)}</tr></thead><tbody>{products.map(product => <tr key={product.wareid}><td><img src={product.image} alt={product.warename} /></td><td>{product.warecode}</td><td title={product.warename}>{product.warename}</td><td>{Number(product.price ?? 0).toFixed(2)}</td><td>{product.specification || "—"}</td><td><input className="combination-quantity-input" aria-label={`${product.warename}数量`} type="number" min="1" step="1" value={product.wareqty ?? 1} onChange={event => onUpdate(product.wareid, { wareqty: Math.max(1, Number(event.target.value) || 1) })} /></td><td><button className="combination-delete-btn" onClick={() => onRemove(product)}>删除</button></td></tr>)}</tbody></table></div>;
}

function CombinationPriceCreate({ setView, onSave }) {
  const [title, setTitle] = useState("组合价"); const [starttime, setStarttime] = useState(""); const [endtime, setEndtime] = useState(""); const [combinationPrice, setCombinationPrice] = useState(""); const [selectedProducts, setSelectedProducts] = useState([]); const [description, setDescription] = useState(""); const [pickerOpen, setPickerOpen] = useState(false); const [message, setMessage] = useState("");
  ...
  const addProducts = products => { setMessage(""); setSelectedProducts(current => { const additions = products.filter(product => !current.some(item => item.wareid === product.wareid)).map(product => ({ ...product, wareqty: 1 })); return current.length + additions.length > 9 ? current : [...current, ...additions]; }); setPickerOpen(false); };
  const save = () => {
    if (!title.trim() || !starttime || !endtime || selectedProducts.length === 0) { setMessage("请完整填写活动信息，并至少选择一个组合商品"); return; }
    if (combinationPrice.trim() === "") { setMessage("请输入组合价格"); return; }
    ...
    if (endtime <= starttime) { setMessage("结束时间必须晚于开始时间"); return; }
  ...
  return <PageFrame crumb={crumb} section="marketing" setView={setView}><section className="panel combination-create-panel"><div className="combination-create-form"><div className="combination-form-row"><label htmlFor="combination-title">活动标题：</label><input id="combination-title" maxLength={100} value={title} onChange={event => { setTitle(event.target.value); setMessage(""); }} /></div><div className="combination-form-row"><label htmlFor="combination-start">开始时间：</label><div className="combination-datetime"><FiClock /><input id="combination-start" value={starttime} onChange={event => { setStarttime(event.target.value); setMessage(""); }} placeholder="请选择日期时间" /></div></div><div className="combination-form-row"><label htmlFor="combination-end">结束时间：</label><div className="combination-datetime"><FiClock /><input id="combination-end" value={endtime} onChange={event => { setEndtime(event.target.value); setMessage(""); }} placeholder="请选择日期时间" /></div></div><div className="combination-form-row"><label htmlFor="combination-price">组合价格：</label><input id="combination-price" className="combination-price-input" type="number" min="0" step="0.01" inputMode="decimal" aria-required="true" aria-invalid={message === "请输入组合价格"} value={combinationPrice} onChange={event => { setCombinationPrice(event.target.value); setMessage(""); }} placeholder="请输入组合价格" /></div><div className="combination-form-row product-row"><label>组合商品：</label><div className="combination-products-field"><button className="secondary product-picker-btn" disabled={selectedProducts.length >= 9} onClick={() => setPickerOpen(true)}>添加组合商品</button>{selectedProducts.length > 0 && <SelectedCombinationProducts products={selectedProducts} onUpdate={updateProduct} onRemove={removeProduct} />}<p>组合商品最多可添加9个</p></div></div><div className="combination-form-row description-row"><label htmlFor="combination-description">组合描述：</label><div><textarea id="combination-description" maxLength={500} value={description} onChange={event => setDescription(event.target.value)} /><span>{description.length}/500</span></div></div><div className="combination-form-actions"><button className="primary" onClick={save}>保存</button>{message && <span className="combination-form-message">{message}</span>}</div></div></section>{pickerOpen && <CombinationProductPicker selectedProducts={selectedProducts} onConfirm={addProducts} onClose={() => setPickerOpen(false)} />}</PageFrame>;
}
