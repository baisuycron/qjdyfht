...
function CombinationDateTimePicker({ id, label, value, opposite, mode, invalid, onChange }) {
  const [open, setOpen] = useState(false); const [visibleMonth, setVisibleMonth] = useState(() => new Date(2026, 7, 1)); const [draftDate, setDraftDate] = useState(""); const [draftTime, setDraftTime] = useState("00:00");
  const dateBound = opposite ? opposite.slice(0, 10) : ""; const timeBound = opposite ? opposite.slice(11, 16) : "";
  const openPicker = () => { const [date = "", time = "00:00"] = value.split(" "); setDraftDate(date); setDraftTime(time.slice(0, 5)); setVisibleMonth(date ? new Date(`${date}T00:00:00`) : new Date(2026, 7, 1)); setOpen(current => !current); };
  const isDisabled = date => mode === "start" ? Boolean(dateBound && dateKey(date) > dateBound) : Boolean(dateBound && dateKey(date) < dateBound);
  const selectNow = () => { const now = new Date(); setDraftDate(dateKey(now)); setDraftTime(`${String(now.getHours()).padStart(2, "0")}:${String(now.getMinutes()).padStart(2, "0")}`); };
  const confirm = () => { if (!draftDate) return; const next = `${draftDate} ${draftTime || "00:00"}:00`; if ((mode === "start" && opposite && next > opposite) || (mode === "end" && opposite && next < opposite)) return; onChange(next); setOpen(false); };
  const timeLimit = draftDate && dateBound === draftDate ? (mode === "start" ? { max: timeBound } : { min: timeBound }) : {};
  return <div className="combination-datetime-picker"><button id={id} type="button" aria-label={label} aria-expanded={open} className={invalid ? "combination-datetime-trigger invalid" : "combination-datetime-trigger"} onClick={openPicker}><FiClock /><span className={value ? "" : "placeholder"}>{value || `请选择${label}`}</span></button>{open && <div className="combination-datetime-popover"><div className="combination-datetime-fields"><input type="text" value={draftDate} placeholder="选择日期" readOnly /><input type="time" value={draftTime} onChange={event => setDraftTime(event.target.value)} {...timeLimit} /></div><div className="combination-calendar-heading"><button type="button" aria-label="上个月" onClick={() => setVisibleMonth(current => new Date(current.getFullYear(), current.getMonth() - 1, 1))}><FiChevronLeft /></button><strong>{monthLabel(visibleMonth)}</strong><button type="button" aria-label="下个月" onClick={() => setVisibleMonth(current => new Date(current.getFullYear(), current.getMonth() + 1, 1))}><FiChevronRight /></button></div><div className="combination-calendar-weekdays">{["日", "一", "二", "三", "四", "五", "六"].map(day => <span key={day}>{day}</span>)}</div><div className="combination-calendar-days">{calendarDays(visibleMonth).map(day => { const current = dateKey(day); const outside = day.getMonth() !== visibleMonth.getMonth(); return <button type="button" key={current} disabled={isDisabled(day)} className={`${outside ? "outside" : ""} ${draftDate === current ? "selected" : ""}`} onClick={() => setDraftDate(current)}>{day.getDate()}</button>; })}</div><footer><button type="button" className="text-btn" onClick={selectNow}>此刻</button><button type="button" className="secondary" onClick={() => setOpen(false)}>取消</button><button type="button" className="primary" disabled={!draftDate} onClick={confirm}>确定</button></footer></div>}</div>;
}
...
function CombinationPriceList({ setView, activities = combinationPriceActivities }) {
  const [activityNo, setActivityNo] = useState(""); const [activityName, setActivityName] = useState(""); const [startDate, setStartDate] = useState(""); const [endDate, setEndDate] = useState(""); const [productName, setProductName] = useState(""); const [productCode, setProductCode] = useState("");
  const [query, setQuery] = useState({ activityNo: "", activityName: "", startDate: "", endDate: "", productName: "", productCode: "" });
  const now = useCurrentTime();
  const [page, setPage] = useState(1); const [pageSize, setPageSize] = useState(10); const [jumpPage, setJumpPage] = useState("1");
  const filteredActivities = activities.filter(activity => (!query.activityNo || activity.warecode === query.activityNo) && (!query.activityName || activity.warename.includes(query.activityName)) && (!query.startDate || activity.endtime >= query.startDate) && (!query.endDate || activity.starttime <= query.endDate) && (!query.productName || activity.itemList.some(item => item.warename.includes(query.productName))) && (!query.productCode || activity.itemList.some(item => item.warecode === query.productCode)));
  ...
  const switchPage = target => { const next = Math.min(Math.max(target, 1), totalPages); setPage(next); setJumpPage(String(next)); };
  const runQuery = () => { setQuery({ activityNo, activityName, startDate, endDate, productName, productCode }); switchPage(1); };
  const reset = () => { setActivityNo(""); setActivityName(""); setStartDate(""); setEndDate(""); setProductName(""); setProductCode(""); setQuery({ activityNo: "", activityName: "", startDate: "", endDate: "", productName: "", productCode: "" }); switchPage(1); };
  return <PageFrame crumb="组合价" section="marketing" setView={setView}><div className="filters panel activity-filter limited-discount-filter combination-price-filter"><label>活动单号<input aria-label="活动单号" maxLength={50} value={activityNo} onChange={event => setActivityNo(event.target.value)} placeholder="请输入活动单号" /></label><label>活动名称<input aria-label="活动名称" maxLength={100} value={activityName} onChange={event => setActivityName(event.target.value)} placeholder="请输入活动名称" /></label><label className="modified-time">活动起止时间<ActivityDateRangePicker start={startDate} end={endDate} onChange={(start, end) => { setStartDate(start); setEndDate(end); }} /></label><label>商品名称<input aria-label="商品名称" maxLength={100} value={productName} onChange={event => setProductName(event.target.value)} placeholder="请输入商品名称" /></label><label>商品编码<input aria-label="商品编码" maxLength={50} value={productCode} onChange={event => setProductCode(event.target.value)} placeholder="请输入商品编码" /></label><div className="filter-actions"><button className="primary" onClick={runQuery}><FiSearch /> 查询</button><button className="secondary" onClick={reset}>重置</button></div></div><section className="panel coupon-table-wrap"><div className="combination-list-actions"><button className="primary" onClick={() => setView("combinationPriceCreate")}><FiPlus /> 新增组合价</button></div><table className="coupon-table combination-price-table"><thead><tr>{["活动单号", "活动名称", "活动时间", "活动状态", "操作"].map(x => <th key={x}>{x}</th>)}</tr></thead><tbody>{rows.map(activity => { const status = getActivityStatus(activity, now); return <tr key={activity.wareid}><td>{activity.warecode || "—"}</td><td>{activity.warename || "—"}</td><td>{activity.starttime && activity.endtime ? `${activity.starttime} ~ ${activity.endtime}` : "—"}</td><td><span className={statusClasses[status]}><i className="status-dot" />{status}</span></td><td><button className="text-btn" onClick={() => setView(["combinationPriceDetail", activity])}>查看</button></td></tr>; })}</tbody></table>{total === 0 ? <p className="empty-row">暂无符合条件的组合价活动</p> : <div className="pagination limited-discount-pagination"><span>共{total}条</span><select aria-label="每页条数" value={pageSize} onChange={e => { setPageSize(Number(e.target.value)); switchPage(1); }}><option value="10">10条/页</option><option value="20">20条/页</option><option value="50">50条/页</option><option value="100">100条/页</option></select><button aria-label="上一页" disabled={currentPage === 1} onClick={() => switchPage(currentPage - 1)}><FiChevronLeft /></button>{Array.from({ length: totalPages }, (_, index) => index + 1).map(pageNo => pageNo === currentPage ? <b key={pageNo}>{pageNo}</b> : <button key={pageNo} onClick={() => switchPage(pageNo)}>{pageNo}</button>)}<button aria-label="下一页" disabled={currentPage === totalPages} onClick={() => switchPage(currentPage + 1)}><FiChevronRight /></button><span>前往</span><input aria-label="跳转页码" value={jumpPage} onChange={e => setJumpPage(e.target.value.replace(/\D/g, ""))} onKeyDown={e => e.key === "Enter" && switchPage(Number(jumpPage) || 1)} onBlur={() => switchPage(Number(jumpPage) || 1)} /><span>页</span></div>}</section></PageFrame>;
}

function CombinationProductPicker({ selectedProducts, onConfirm, onClose }) {
  ...
  return <div className="modal-backdrop product-select-backdrop">
    <section className="modal combination-product-modal" aria-label="商品选择">
      ...
      <div className="product-select-table-wrap"><table className="product-select-table"><thead><tr><th>商品编码</th><th>商品名称</th><th>最新售价</th></tr></thead><tbody>{rows.map(product => { const checked = pendingIds.includes(product.wareid); return <tr key={product.wareid} className={checked ? "selected" : ""}><td><div className="product-code-cell"><input type="checkbox" aria-label={`选择${product.warename}`} checked={checked} onChange={() => toggleProduct(product)} /><span>{product.warecode}</span></div></td><td><div className="product-name-cell"><img src={product.image} alt="" /><span>{product.warename}</span></div></td><td>{product.price.toFixed(product.price % 1 === 0 ? 1 : 2)}</td></tr>; })}</tbody></table>{rows.length === 0 && <p className="product-select-empty">暂无符合条件的商品</p>}</div>{limitToast && <div className="product-select-limit-toast" role="alert"><span aria-hidden="true">×</span>组合商品最多只能选择9个~</div>}
...
function SelectedCombinationProducts({ products, onUpdate, onRemove }) {
  return <div className="combination-selected-table-wrap"><table className="combination-selected-table"><thead><tr>{["商品图片", "商品编码", "商品名称", "最新售价", "商品规格", "数量", "操作"].map(title => <th key={title}>{title}</th>)}</tr></thead><tbody>{products.map(product => <tr key={product.wareid}><td><img src={product.image} alt={product.warename} /></td><td>{product.warecode}</td><td title={product.warename}>{product.warename}</td><td>{Number(product.price ?? 0).toFixed(2)}</td><td>{product.specification || "—"}</td><td><input className="combination-quantity-input" aria-label={`${product.warename}数量`} type="number" min="1" step="1" value={product.wareqty ?? 1} onChange={event => onUpdate(product.wareid, { wareqty: Math.max(1, Number(event.target.value) || 1) })} /></td><td><button className="combination-delete-btn" onClick={() => onRemove(product)}>删除</button></td></tr>)}</tbody></table></div>;
}

function CombinationPriceCreate({ setView, onSave }) {
  const [title, setTitle] = useState(""); const [starttime, setStarttime] = useState(""); const [endtime, setEndtime] = useState(""); const [combinationPrice, setCombinationPrice] = useState(""); const [selectedProducts, setSelectedProducts] = useState([]); const [description, setDescription] = useState(""); const [pickerOpen, setPickerOpen] = useState(false); const [message, setMessage] = useState(""); const [validationAttempted, setValidationAttempted] = useState(false); const [formToastMessage, setFormToastMessage] = useState("");
  ...
  const addProducts = products => { setMessage(""); setSelectedProducts(current => products.map(product => current.find(item => item.wareid === product.wareid) || { ...product, wareqty: 1 })); setPickerOpen(false); };
  const showFormToast = text => { setFormToastMessage(text); window.setTimeout(() => setFormToastMessage(""), 1800); };
  const save = () => {
    setValidationAttempted(true);
    if (!title.trim() || !starttime || !endtime || combinationPrice.trim() === "") { setMessage(""); return; }
    if (!/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(starttime) || !/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(endtime)) { setMessage("时间格式应为：YYYY-MM-DD HH:mm:ss"); return; }
    if (endtime <= starttime) { setMessage(""); showFormToast("结束时间必须晚于开始时间"); return; }
    if (selectedProducts.length === 0) { showFormToast("请添加组合商品"); return; }
    onSave({ wareid: Date.now(), warecode: `NEW${String(Date.now()).slice(-7)}`, warename: title.trim(), starttime, endtime, combinationPrice: Number(combinationPrice), marketingType: "组合价", cnt: selectedProducts.length, itemList: selectedProducts.map(item => ({ ...item, wareqty: item.wareqty ?? 1 })), description });
  };
  const titleInvalid = validationAttempted && !title.trim(); const startInvalid = validationAttempted && !starttime; const endInvalid = validationAttempted && !endtime; const priceInvalid = validationAttempted && !combinationPrice.trim();
  const crumb = <><button className="breadcrumb-link" onClick={() => setView("combinationPrice")}>组合价</button><span> › 新增组合价</span></>;
  return <PageFrame crumb={crumb} section="marketing" setView={setView}>
    <section className="panel combination-create-panel"><div className="combination-create-form">
      <div className="combination-form-row"><label className="required-label" htmlFor="combination-title">活动名称：</label><div className="combination-field-control"><input id="combination-title" maxLength={100} value={title} aria-required="true" aria-invalid={titleInvalid} onChange={event => { setTitle(event.target.value); setMessage(""); }} placeholder="请输入活动名称" />{titleInvalid && <span className="combination-field-error">请输入活动名称</span>}</div></div>
      <div className="combination-form-row"><label className="required-label" htmlFor="combination-start">开始时间：</label><div className="combination-field-control"><CombinationDateTimePicker id="combination-start" label="开始时间" value={starttime} opposite={endtime} mode="start" invalid={startInvalid} onChange={value => { setStarttime(value); setMessage(""); }} />{startInvalid && <span className="combination-field-error">请选择开始时间</span>}</div></div>
      <div className="combination-form-row"><label className="required-label" htmlFor="combination-end">结束时间：</label><div className="combination-field-control"><CombinationDateTimePicker id="combination-end" label="结束时间" value={endtime} opposite={starttime} mode="end" invalid={endInvalid} onChange={value => { setEndtime(value); setMessage(""); }} />{endInvalid && <span className="combination-field-error">请选择结束时间</span>}</div></div>
      <div className="combination-form-row"><label className="required-label" htmlFor="combination-price">组合价格：</label><div className="combination-field-control"><input id="combination-price" className="combination-price-input" type="number" min="0" step="0.01" inputMode="decimal" aria-required="true" aria-invalid={priceInvalid} value={combinationPrice} onChange={event => { setCombinationPrice(event.target.value); setMessage(""); }} placeholder="请输入组合价格" />{priceInvalid && <span className="combination-field-error">请输入组合价格</span>}</div></div>
      <div className="combination-form-row product-row"><label className="required-label">组合商品：</label><div className="combination-products-field"><button className="secondary product-picker-btn" onClick={() => setPickerOpen(true)}>添加组合商品</button>{selectedProducts.length > 0 && <SelectedCombinationProducts products={selectedProducts} onUpdate={updateProduct} onRemove={removeProduct} />}<p>组合商品最多可添加9个</p></div></div>
      <div className="combination-form-row description-row"><label htmlFor="combination-description">组合描述：</label><div><textarea id="combination-description" maxLength={500} value={description} onChange={event => setDescription(event.target.value)} /><span>{description.length}/500</span></div></div>
      <div className="combination-form-actions"><button className="primary" onClick={save}>保存</button>{message && <span className="combination-form-message">{message}</span>}</div>
    </div></section>{formToastMessage && <div className="combination-form-toast" role="alert"><span aria-hidden="true">×</span>{formToastMessage}</div>}{pickerOpen && <CombinationProductPicker selectedProducts={selectedProducts} onConfirm={addProducts} onClose={() => setPickerOpen(false)} />}
  </PageFrame>;
}
