...
function CombinationPriceList({ setView }) {
  const [combinationCode, setCombinationCode] = useState(""); const [combinationName, setCombinationName] = useState("");
  const [query, setQuery] = useState({ combinationCode: "", combinationName: "" });
  const [page, setPage] = useState(1); const [pageSize, setPageSize] = useState(10); const [jumpPage, setJumpPage] = useState("1");
  const filteredActivities = combinationPriceActivities.filter(activity => (!query.combinationCode || activity.warecode === query.combinationCode) && (!query.combinationName || activity.warename.includes(query.combinationName)));
  ...
  const switchPage = target => { const next = Math.min(Math.max(target, 1), totalPages); setPage(next); setJumpPage(String(next)); };
  const runQuery = () => { setQuery({ combinationCode, combinationName }); switchPage(1); };
  const reset = () => { setCombinationCode(""); setCombinationName(""); setQuery({ combinationCode: "", combinationName: "" }); switchPage(1); };
  return <PageFrame crumb="组合价" section="marketing" setView={setView}><div className="filters panel activity-filter limited-discount-filter combination-price-filter"><label>组合编码<input aria-label="组合编码" maxLength={50} value={combinationCode} onChange={event => setCombinationCode(event.target.value)} placeholder="请输入组合编码" /></label><label>组合名称<input aria-label="组合名称" maxLength={100} value={combinationName} onChange={event => setCombinationName(event.target.value)} placeholder="请输入组合名称" /></label><div className="filter-actions"><button className="primary" onClick={runQuery}><FiSearch /> 查询</button><button className="secondary" onClick={reset}>重置</button></div></div><section className="panel coupon-table-wrap"><table className="coupon-table combination-price-table"><thead><tr>{["组合编码", "组合名称", "规格/单位", "厂家", "组成商品数", "操作"].map(x => <th key={x}>{x}</th>)}</tr></thead><tbody>{rows.map(activity => <tr key={activity.wareid}><td>{activity.warecode || "—"}</td><td>{activity.warename || "—"}</td><td>{[activity.warespec, activity.wareunit].filter(Boolean).join(" / ") || "—"}</td><td>{activity.factoryname || "—"}</td><td>{activity.cnt ?? activity.itemList?.length ?? 0}</td><td><button className="text-btn" onClick={() => setView(["combinationPriceDetail", activity])}>查看</button></td></tr>)}</tbody></table>{total === 0 ? <p className="empty-row">暂无符合条件的组合价活动</p> : <div className="pagination limited-discount-pagination"><span>共{total}条</span><select aria-label="每页条数" value={pageSize} onChange={e => { setPageSize(Number(e.target.value)); switchPage(1); }}><option value="10">10条/页</option><option value="20">20条/页</option><option value="50">50条/页</option><option value="100">100条/页</option></select><button aria-label="上一页" disabled={currentPage === 1} onClick={() => switchPage(currentPage - 1)}><FiChevronLeft /></button>{Array.from({ length: totalPages }, (_, index) => index + 1).map(pageNo => pageNo === currentPage ? <b key={pageNo}>{pageNo}</b> : <button key={pageNo} onClick={() => switchPage(pageNo)}>{pageNo}</button>)}<button aria-label="下一页" disabled={currentPage === totalPages} onClick={() => switchPage(currentPage + 1)}><FiChevronRight /></button><span>前往</span><input aria-label="跳转页码" value={jumpPage} onChange={e => setJumpPage(e.target.value.replace(/\D/g, ""))} onKeyDown={e => e.key === "Enter" && switchPage(Number(jumpPage) || 1)} onBlur={() => switchPage(Number(jumpPage) || 1)} /><span>页</span></div>}</section></PageFrame>;
}
