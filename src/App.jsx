...
function CombinationPriceList({ setView, activities = combinationPriceActivities, onTerminate }) {
  ...
  return <PageFrame crumb="组合价" section="marketing" setView={setView}>
    ...
    {terminateTarget && <div className="modal-backdrop"><section className="modal combination-terminate-modal" role="dialog" aria-modal="true" aria-label="结束活动确认"><header><b>结束活动</b><button aria-label="关闭结束活动确认" onClick={() => setTerminateTarget(null)}><FiX /></button></header><div className="combination-terminate-content"><strong>确认要结束“{terminateTarget.warename}”活动吗？</strong></div><footer><button className="secondary" onClick={() => setTerminateTarget(null)}>取消</button><button className="danger-primary" onClick={confirmTerminate}>确认结束</button></footer></section></div>}
