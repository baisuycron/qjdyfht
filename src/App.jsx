...
const storePickerOptions = [...new Map(Object.entries(storeNamesByCode).map(([code, name]) => [name, { code, name }])).values()];
...
const erpPromotionActivities = {
  ...
  "满额+XX元换购": [
    { promName: "满88元9.9元换购护手霜", pstplanno: "202608110001", displayType: "满额换购", marketingType: "满减满赠", givetype: 1, starttime: "2026-08-01 00:00:00", endtime: "2026-08-31 23:59:59", busnos: "全部", days: "全部", weekdays: "1111111", sumamt: 88, sumqty: 0, giveprice: 0, givenum: 1, plannum: 0, conditionItemList: [{ rowno: 1, wareid: 0, sumamt: 0, sumqty: 0, profitrate: 0, resprice: 0 }], giftItemList: [{ rowno: 1, pstid: 200105, pstqty: 1, priceDisc: 0, pstprice: 9.9 }] },
    { promName: "满199元19.9元换购家庭药箱", pstplanno: "202608110002", displayType: "满额换购", marketingType: "满减满赠", givetype: 1, starttime: "2026-08-08 00:00:00", endtime: "2026-08-28 23:59:59", busnos: "101001,101002,101008,101010", days: "全部", weekdays: "1111111", sumamt: 199, sumqty: 0, giveprice: 0, givenum: 1, plannum: 0, conditionItemList: [{ rowno: 1, wareid: 0, sumamt: 0, sumqty: 0, profitrate: 0, resprice: 0 }], giftItemList: [{ rowno: 1, pstid: 200208, pstqty: 1, priceDisc: 0, pstprice: 19.9 }] },
    { promName: "口腔护理满59元5元换购牙线", pstplanno: "202608110003", displayType: "满额换购", marketingType: "满减满赠", givetype: 1, starttime: "2026-08-15 00:00:00", endtime: "2026-08-30 23:59:59", busnos: "101004,101007", days: "15,20,25,30", weekdays: "1111111", sumamt: 59, sumqty: 0, giveprice: 0, givenum: 1, plannum: 0, conditionItemList: [{ rowno: 1, wareid: 100824, sumamt: 59, sumqty: 0, profitrate: 0.15, resprice: 1 }], giftItemList: [{ rowno: 1, pstid: 200319, pstqty: 1, priceDisc: 0, pstprice: 5 }] },
  ],
  "买X送Y": [
...
// The existing full-reduction entry includes all four givetype=1 promotion forms.
const quantityExchangeActivities = [
  { ...erpPromotionActivities["买X送Y"][0], promName: "医用口罩买2盒加1元换购1盒", pstplanno: "202609090001", displayType: "买X件+XX元换购", marketingType: "满减满赠", starttime: "2026-09-01 00:00:00", endtime: "2026-09-30 23:59:59", giftItemList: [{ rowno: 1, pstid: 100935, pstqty: 1, priceDisc: 0, pstprice: 1 }] },
  { ...erpPromotionActivities["买X送Y"][1], promName: "儿童退热贴买3盒加0.01元换购1盒", pstplanno: "202609090002", displayType: "买X件+XX元换购", marketingType: "满减满赠", starttime: "2026-09-01 00:00:00", endtime: "2026-09-30 23:59:59", giftItemList: [{ rowno: 1, pstid: 101043, pstqty: 1, priceDisc: 0, pstprice: 0.01 }] },
];
const fullReductionActivities = [
  erpPromotionActivities["满减满赠"][1],
  ...erpPromotionActivities["满额+XX元换购"],
  ...quantityExchangeActivities,
  ...erpPromotionActivities["买X送Y"],
  ...erpPromotionActivities["满减满赠"].filter((_, index) => index !== 1),
].filter(activity => !activity.giftItemList.some(item => Number(item.priceDisc) === 1)).map(activity => ({ ...activity, displayType: activity.givetype === 6 ? "满减" : activity.sumamt > 0 ? (activity.giftItemList.some(item => item.priceDisc === 0 && item.pstprice > 0) ? "满额+XX元换购" : "满减满赠") : activity.giftItemList.some(item => item.priceDisc === 0 && item.pstprice > 0) ? "买X件+XX元换购" : "买X送Y" }));
...
const erpPromotionLastModifiedTimes = {
  "202609090001": "2026-09-09 09:00:00", "202609090002": "2026-09-09 09:05:00",
...
const isExchangePromotion = activity => activity.givetype === 1 && activity.giftItemList.some(item => item.priceDisc === 0 && Number(item.pstprice) > 0);
const promotionModeText = activity => activity.givetype === 6 ? "整单减金额" : isExchangePromotion(activity) ? "加价换购" : activity.givetype === 2 ? "条件商品价格优惠" : "赠送商品";
const promotionRuleText = activity => {
  ...
  if (isExchangePromotion(activity)) return `${mainThreshold || `买${condition.sumqty}件`}，${gift.pstprice}元换购${gift.pstqty}件`;
...
const giftSelectionText = activity => {
  ...
  if (isExchangePromotion(activity)) return "固定换购商品";
...
const scopeItemText = item => Number(item.wareid) === 0 ? "所有商品（逐个判断）" : item.wareid;
...
function StoreNamePicker({ value, search, onChange, onSearch }) {
  const [open, setOpen] = useState(false); const [editingSelected, setEditingSelected] = useState(false);
  const searchTerm = editingSelected && search === value ? "" : search;
  const matches = storePickerOptions.filter(store => store.name.toLowerCase().includes(searchTerm.trim().toLowerCase()));
  const selectStore = store => { onChange(store.name); onSearch(""); setEditingSelected(false); setOpen(false); };
  if (value && !editingSelected) return <div className="store-name-picker"><div className="store-picker-selection" role="button" tabIndex="0" onClick={() => { setEditingSelected(true); onSearch(value); setOpen(true); }} onKeyDown={event => event.key === "Enter" && (setEditingSelected(true), onSearch(value), setOpen(true))}><span>{value}</span><button type="button" aria-label="清除已选门店" onClick={event => { event.stopPropagation(); onChange(""); onSearch(""); setEditingSelected(false); setOpen(false); }}><FiX /></button></div></div>;
  return <div className="store-name-picker"><input autoFocus={editingSelected} aria-label="门店名称" maxLength={100} value={search} onFocus={() => setOpen(true)} onChange={event => { onSearch(event.target.value); setOpen(true); }} onKeyDown={event => event.key === "Escape" && (setEditingSelected(false), setOpen(false))} placeholder="请输入门店名称" />{open && <div className="store-picker-dropdown" role="listbox" aria-label="匹配门店">{matches.length ? matches.map(store => <button key={store.code} className={store.name === value ? "selected" : ""} type="button" role="option" aria-selected={store.name === value} onMouseDown={event => event.preventDefault()} onClick={() => selectStore(store)}><span>{store.name}</span></button>) : <p>暂无匹配门店</p>}</div>}</div>;
}

function ClearableFilterSelect({ label, value, options, onChange }) {
  const [open, setOpen] = useState(false);
  const selectedOption = options.find(option => option.value === value);
  return <div className="clearable-filter-select"><button type="button" className="clearable-filter-trigger" aria-label={label} aria-expanded={open} onClick={() => setOpen(current => !current)}><span className={value ? "" : "placeholder"}>{selectedOption?.label || "请选择"}</span><FiChevronDown /></button>{value && <button type="button" className="clearable-filter-clear" aria-label={`清空${label}`} onClick={() => { onChange(""); setOpen(false); }}><FiX /></button>}{open && <div className="clearable-filter-options" role="listbox" aria-label={`${label}选项`}>{options.map(option => <button key={option.value} type="button" role="option" aria-selected={option.value === value} className={option.value === value ? "selected" : ""} onClick={() => { onChange(option.value); setOpen(false); }}>{option.label}</button>)}</div>}</div>;
}
...
function LimitedDiscountList({ setView }) {
  ...
  const [activityName, setActivityName] = useState(""); const [activityNo, setActivityNo] = useState(""); const [activityStatus, setActivityStatus] = useState(""); const [storeName, setStoreName] = useState(""); const [storeSearch, setStoreSearch] = useState(""); const [storePickerKey, setStorePickerKey] = useState(0); const [discountType, setDiscountType] = useState("");
...
function ErpPromotionList({ setView, category }) {
  ...
  useEffect(() => setPage(1), [query, pageSize, category]);
  const activities = category === "满减满赠" ? fullReductionActivities : erpPromotionActivities[category];
  const [activitySyncTime, setActivitySyncTime] = useState("2026-08-25 08:30:00");
  const supportsActivityType = category === "满减满赠";
  const activityTypeOptions = [...new Set(activities.map(activity => activity.displayType))];
  const rows = activities.filter(activity => (!query.start || activity.endtime >= `${query.start} 00:00:00`) && (!query.end || activity.starttime <= `${query.end} 23:59:59`) && (!query.activityName || activity.promName.toLowerCase().includes(query.activityName.toLowerCase())) && (!query.activityNo || activity.pstplanno === query.activityNo) && (!query.activityStatus || query.activityStatus === "全部" || getActivityStatus(activity, now) === query.activityStatus) && matchesStoreName(activity.busnos, query.storeName) && (!supportsActivityType || !query.activityType || activity.displayType === query.activityType));
  const totalPages = Math.max(1, Math.ceil(rows.length / pageSize));
  const currentPage = Math.min(page, totalPages);
...
const calculatePromotionTrial = (activity, values) => {
  ...
  const conditionRows = activity.conditionItemList.map(item => {
    ...
    const amount = Number(entered.amount);
    const qty = Number(entered.qty);
  ...
  else if (eligible && isExchangePromotion(activity)) outcome = `${firstBenefit.pstprice} 元换购 ${firstBenefitName} × ${firstBenefit.pstqty * repeatTimes}`;
...
function CombinationPriceList({ setView, activities = combinationPriceActivities, onToggleStatus, onDelete }) {
  ...
  const toggleStatus = (activity, confirmed = false) => {
    if (!confirmed) { setStatusTarget(activity); return; }
  ...
  return <PageFrame crumb="组合价" section="marketing" setView={setView}>
    ...
    {statusTarget && <div className="modal-backdrop"><section className="modal combination-delete-modal" role="dialog" aria-modal="true" aria-label={`${statusTarget.enabled ? "停用" : "启用"}组合价活动`}><header><b>{statusTarget.enabled ? "停用" : "启用"}组合价活动</b><button aria-label="关闭状态确认" onClick={() => setStatusTarget(null)}><FiX /></button></header><div className="combination-delete-content"><strong>确认{statusTarget.enabled ? "停用" : "启用"}“{statusTarget.warename}”吗？</strong></div><footer><button className="secondary" onClick={() => setStatusTarget(null)}>取消</button><button className="danger-primary" onClick={() => toggleStatus(statusTarget, true)}>确认</button></footer></section></div>}
...
function CombinationPriceForm({ setView, onSave, existingActivities = [], activity = null, mode = "create" }) {
  const readOnly = mode === "view" || (mode === "edit" && activity?.enabled); const isEdit = mode === "edit";
