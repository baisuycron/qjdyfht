...
function ErpPromotionDetail({ setView, category, activity }) {
  ...
  return <PageFrame crumb={`门店促销 › ${category} › 活动详情`} section="marketing" setView={setView}><button className="back-btn" onClick={() => setView(["erpPromotion", category])}>返回列表</button><section className="panel coupon-detail activity-detail"><div className="detail-heading"><div><h3>基本信息</h3><h4>ERP 活动规则</h4></div><span className="erp-readonly-badge">ERP 同步 · 只读</span></div><div className="detail-grid"><Info label="活动名称" value={activity.promName} simple /><Info label="活动单号" value={activity.pstplanno} simple /><Info label="活动类型" value={activity.displayType} simple /><Info label="促销方式" value={promotionModeText(activity)} simple /><Info label="活动时间" value={`${activity.starttime} ~ ${activity.endtime}`} simple /><Info label="适用门店" value={storeScopeText(activity.busnos)} simple /><Info label="参与日期" value={dateScope} simple /><Info label="参与星期" value={weekdayText(activity.weekdays)} simple /></div></section><section className="panel promotion-rule-panel"><div className="discount-items-title"><div><h3>活动门槛与优惠</h3><p>主表门槛与条件商品要求同时满足</p></div></div><div className="promotion-rule-grid"><div><span>规则摘要</span><strong>{promotionRuleText(activity)}</strong></div><div><span>整单金额门槛</span><strong>{activity.sumamt ? `${activity.sumamt}元` : "不限"}</strong></div><div><span>整单数量门槛</span><strong>{activity.sumqty ? `${activity.sumqty}件` : "不限"}</strong></div><div><span>条件关系</span><strong>{activity.plannum === 1 ? "任一条件满足" : "全部条件满足"}</strong></div>{activity.givetype === 6 && <div className="benefit-highlight"><span>整单减金额</span><strong>{activity.giveprice}元</strong></div>}</div></section><section className="panel discount-items-panel"><div className="discount-items-title"><div><h3>条件商品明细</h3><p>商品编码为 0 表示全部商品</p></div><span>共 {activity.conditionItemList.length} 条</span></div><table className="coupon-table promotion-condition-table"><thead><tr><th>行号</th><th>条件商品</th><th>单品金额门槛</th><th>单品数量门槛</th><th>毛利率下限</th><th>恢复原价</th></tr></thead><tbody>{activity.conditionItemList.map(item => <tr key={item.rowno}><td>{item.rowno}</td><td>{scopeItemText(item)}</td><td>{requirementText(item.sumamt)}{Number(item.sumamt) > 0 && "元"}</td><td>{requirementText(item.sumqty)}{Number(item.sumqty) > 0 && "件"}</td><td>{Number(item.profitrate) > 0 ? `${item.profitrate * 100}%` : "不限"}</td><td>{item.resprice === 1 ? "是" : "否"}</td></tr>)}</tbody></table></section>{activity.givetype !== 6 && <section className="panel discount-items-panel"><div className="discount-items-title"><div><h3>{activity.givetype === 2 ? "换购商品明细" : "赠送商品明细"}</h3><p>{activity.givetype === 2 ? "展示换购商品成交价格" : "展示赠品及赠送数量"}</p></div><span>共 {activity.giftItemList.length} 个商品</span></div><table className="coupon-table promotion-gift-table"><thead><tr><th>行号</th><th>商品编码</th><th>数量</th><th>优惠计价</th><th>成交价格 / 折扣</th></tr></thead><tbody>{activity.giftItemList.map(item => <tr key={item.rowno}><td>{item.rowno}</td><td>{item.pstid}</td><td>{item.pstqty}</td><td>{item.priceDisc === 1 ? "按折扣" : "按价格"}</td><td>{item.priceDisc === 1 ? `${item.pstprice * 10}折` : `${item.pstprice}元`}</td></tr>)}</tbody></table></section>}</PageFrame>;
}

const combinationPriceActivities = [
  { wareid: 84791, warecode: "ZH32958815", warename: "可孚组合套装9.9元", warespec: "1组", wareunit: "组", factoryname: "可孚", marketingType: "组合价", lasttime: "2026-08-06 09:32:18", itemList: [{ wareid: 100001, warecode: "072256", warename: "可孚棉片（珍珠纹）", warespec: "50片", wareqty: 1 }, { wareid: 100032, warecode: "068619", warename: "可孚棉签", warespec: "竹棒型（10cm*50支）", wareqty: 1 }, { wareid: 100035, warecode: "073977", warename: "可孚牙科清洁器", warespec: "KF-JZ003，30支独立装", wareqty: 1 }] },
  { wareid: 84792, warecode: "ZH32958816", warename: "家庭常备护理组合", warespec: "1组", wareunit: "组", factoryname: "可孚", marketingType: "组合价", lasttime: "2026-08-06 09:28:04", itemList: [{ wareid: 100103, warecode: "078241", warename: "医用棉球", warespec: "50g", wareqty: 2 }, { wareid: 100115, warecode: "079675", warename: "创可贴", warespec: "经济型 100片", wareqty: 1 }] },
  { wareid: 85120, warecode: "ZH32958902", warename: "夏日防暑护理组合", warespec: "1组", wareunit: "组", factoryname: "千金药业", marketingType: "组合价", lasttime: "2026-08-05 16:40:23", itemList: [{ wareid: 100212, warecode: "080114", warename: "藿香正气口服液", warespec: "10ml*10支", wareqty: 1 }, { wareid: 100218, warecode: "080356", warename: "清凉油", warespec: "3g", wareqty: 2 }] },
  { wareid: 85124, warecode: "ZH32958915", warename: "儿童出行安心组合", warespec: "1组", wareunit: "组", factoryname: "千金药业", marketingType: "组合价", lasttime: "2026-08-05 14:12:08", itemList: [{ wareid: 100315, warecode: "081923", warename: "儿童退热贴", warespec: "2片*3袋", wareqty: 1 }, { wareid: 100316, warecode: "081924", warename: "儿童口罩", warespec: "10只", wareqty: 1 }, { wareid: 100319, warecode: "081927", warename: "碘伏棉签", warespec: "50支", wareqty: 1 }] },
];

function CombinationPriceList({ setView }) {
  const [modifiedStart, setModifiedStart] = useState(""); const [modifiedEnd, setModifiedEnd] = useState("");
  const [query, setQuery] = useState({ modifiedStart: "", modifiedEnd: "" }); const [pageSize, setPageSize] = useState(10);
  const rows = combinationPriceActivities.filter(activity => (!query.modifiedStart || activity.lasttime >= `${query.modifiedStart} 00:00:00`) && (!query.modifiedEnd || activity.lasttime <= `${query.modifiedEnd} 23:59:59`));
  const reset = () => { setModifiedStart(""); setModifiedEnd(""); setQuery({ modifiedStart: "", modifiedEnd: "" }); };
  return <PageFrame crumb="门店促销 › 组合价" section="marketing" setView={setView}><div className="filters panel activity-filter limited-discount-filter"><label className="modified-time">最后修改时间<ActivityDateRangePicker start={modifiedStart} end={modifiedEnd} onChange={(start, end) => { setModifiedStart(start); setModifiedEnd(end); }} /></label><div className="filter-actions"><button className="primary" onClick={() => setQuery({ modifiedStart, modifiedEnd })}><FiSearch /> 查询</button><button className="secondary" onClick={reset}>重置</button></div></div><section className="panel coupon-table-wrap"><table className="coupon-table combination-price-table"><thead><tr>{["组合编码", "组合名称", "规格/单位", "厂家", "营销类型", "组成商品数", "最后修改时间", "操作"].map(x => <th key={x}>{x}</th>)}</tr></thead><tbody>{rows.slice(0, pageSize).map(activity => <tr key={activity.wareid}><td>{activity.warecode}</td><td>{activity.warename}</td><td>{activity.warespec} / {activity.wareunit}</td><td>{activity.factoryname}</td><td><b className="discount-rule">{activity.marketingType}</b></td><td>{activity.itemList.length}</td><td>{activity.lasttime}</td><td><button className="text-btn" onClick={() => setView(["combinationPriceDetail", activity])}>查看</button></td></tr>)}</tbody></table>{rows.length === 0 ? <p className="empty-row">暂无符合条件的组合价活动</p> : <div className="pagination limited-discount-pagination"><span>共{rows.length}条</span><select aria-label="每页条数" value={pageSize} onChange={e => setPageSize(Number(e.target.value))}><option value="10">10条/页</option><option value="20">20条/页</option><option value="50">50条/页</option></select><button aria-label="上一页" disabled><FiChevronLeft /></button><b>1</b><button aria-label="下一页" disabled><FiChevronRight /></button></div>}</section></PageFrame>;
}

function CombinationPriceDetail({ setView, activity }) {
  return <PageFrame crumb="门店促销 › 组合价 › 活动详情" section="marketing" setView={setView}><button className="back-btn" onClick={() => setView("combinationPrice")}>返回列表</button><section className="panel coupon-detail activity-detail"><div className="detail-heading"><div><h3>基本信息</h3><h4>组合商品</h4></div><span className="erp-readonly-badge">ERP 同步 · 只读</span></div><div className="detail-grid"><Info label="组合名称" value={activity.warename} simple /><Info label="组合编码" value={activity.warecode} simple /><Info label="组合ID" value={activity.wareid} simple /><Info label="营销类型" value={activity.marketingType} simple /><Info label="组合规格" value={activity.warespec} simple /><Info label="组合单位" value={activity.wareunit} simple /><Info label="组合厂家" value={activity.factoryname} simple /><Info label="最后修改时间" value={activity.lasttime} simple /></div></section><section className="panel discount-items-panel"><div className="discount-items-title"><div><h3>组合商品明细</h3><p>按每组组合商品所需数量展示</p></div><span>共 {activity.itemList.length} 个商品</span></div><table className="coupon-table combination-item-table"><thead><tr><th>商品ID</th><th>商品编码</th><th>商品名称</th><th>商品规格</th><th>每组数量</th></tr></thead><tbody>{activity.itemList.map(item => <tr key={item.wareid}><td>{item.wareid}</td><td>{item.warecode}</td><td>{item.warename}</td><td>{item.warespec}</td><td>{item.wareqty}</td></tr>)}</tbody></table></section></PageFrame>;
}
