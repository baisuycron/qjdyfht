...
const combinationPriceActivities = [
  { wareid: 84791, warecode: "0084791", warename: "日常清洁护理组合", starttime: "2026-08-01 00:00:00", endtime: "2026-08-31 23:59:59", combinationPrice: 29.8, marketingType: "组合价", description: "日常清洁与咽喉护理用品组合。", cnt: 3, itemList: [combinationActivityProduct(6947, 1, 0.01), combinationActivityProduct(37692, 1, 22), combinationActivityProduct(37848, 1, 7.79)] },
  { wareid: 84792, warecode: "0084792", warename: "家庭常备护理组合", starttime: "2026-08-10 00:00:00", endtime: "2026-09-09 23:59:59", combinationPrice: 89.9, marketingType: "组合价", description: "家庭常备滋补与护理商品组合。", cnt: 2, itemList: [combinationActivityProduct(8520, 1, 70), combinationActivityProduct(25216, 1, 19.9)] },
  { wareid: 85120, warecode: "0085120", warename: "夏日防暑护理组合", starttime: "2026-07-15 00:00:00", endtime: "2026-08-25 23:59:59", combinationPrice: 29.9, marketingType: "组合价", description: "适用于夏季防暑与咽喉日常护理。", cnt: 2, itemList: [combinationActivityProduct(100212, 1, 15), combinationActivityProduct(37848, 2, 7.45)] },
  { wareid: 85124, warecode: "0085124", warename: "儿童出行护理组合", starttime: "2026-09-01 00:00:00", endtime: "2026-09-30 23:59:59", combinationPrice: 29.8, marketingType: "组合价", description: "儿童出行常备用品组合。", cnt: 3, itemList: [combinationActivityProduct(100315, 1, 10), combinationActivityProduct(100316, 1, 5.8), combinationActivityProduct(100319, 1, 14)] },
  { wareid: 85125, warecode: "0085125", warename: "春季营养关爱组合", starttime: "2026-03-01 00:00:00", endtime: "2026-03-31 23:59:59", combinationPrice: 79.9, marketingType: "组合价", description: "已结束活动样本，用于查看历史组合价配置。", cnt: 2, itemList: [combinationActivityProduct(48488, 1, 79.88), combinationActivityProduct(23001, 2, 0.01)] },
];
...
function CombinationPriceForm({ setView, onSave, existingActivities = [], activity = null, mode = "create" }) {
  ...
  const blockedProductReasons = useMemo(() => {
    const reasons = {};
    existingActivities.filter(item => item.wareid !== activity?.wareid && getActivityStatus(item) !== "已结束").forEach(item => (item.itemList || []).forEach(product => { reasons[product.wareid] = "已参与其他组合价"; }));
  ...
  const save = () => {
    ...
    if (!title.trim() || !starttime || !endtime) { setMessage(""); return; }
    ...
    onSave({ ...activityWithoutDescription, wareid: activity?.wareid ?? Date.now(), warecode: activity?.warecode || nextCombinationActivityNo(existingActivities), warename: title.trim(), starttime, endtime, combinationPrice: Number(combinationPrice.toFixed(2)), marketingType: "组合价", cnt: selectedProducts.length, itemList: selectedProducts.map(item => ({ ...item, wareqty: item.wareqty ?? 1, combinationUnitPrice: Number(item.combinationUnitPrice) })) });
  };
  const titleInvalid = validationAttempted && !title.trim(); const startInvalid = validationAttempted && !starttime; const endInvalid = validationAttempted && !endtime; const priceInvalid = validationAttempted && selectedProducts.length === 0;
