...
function CombinationPriceList({ setView, activities = combinationPriceActivities, onTerminate }) {
  const [activityNo, setActivityNo] = useState(""); const [activityName, setActivityName] = useState(""); const [activityStatus, setActivityStatus] = useState(""); const [startDate, setStartDate] = useState(""); const [endDate, setEndDate] = useState(""); const [productName, setProductName] = useState(""); const [productCode, setProductCode] = useState("");
  const [query, setQuery] = useState({ activityNo: "", activityName: "", activityStatus: "", startDate: "", endDate: "", productName: "", productCode: "" });
  const now = useCurrentTime();
  const [page, setPage] = useState(1); const [pageSize, setPageSize] = useState(10); const [jumpPage, setJumpPage] = useState("1");
  const [terminateTarget, setTerminateTarget] = useState(null); const [toastMessage, setToastMessage] = useState("");
  const filteredActivities = activities.filter(activity => (!query.activityNo || activity.warecode === query.activityNo) && (!query.activityName || activity.warename.includes(query.activityName)) && (!query.activityStatus || query.activityStatus === "全部" || getActivityStatus(activity, now) === query.activityStatus) && (!query.startDate || activity.endtime >= query.startDate) && (!query.endDate || activity.starttime <= query.endDate) && (!query.productName || (activity.itemList || []).some(item => item.warename.includes(query.productName))) && (!query.productCode || (activity.itemList || []).some(item => item.warecode === query.productCode)));
  ...
  const switchPage = target => { const next = Math.min(Math.max(target, 1), totalPages); setPage(next); setJumpPage(String(next)); };
  const runQuery = () => { setQuery({ activityNo, activityName, activityStatus, startDate, endDate, productName, productCode }); switchPage(1); };
  const reset = () => { setActivityNo(""); setActivityName(""); setActivityStatus(""); setStartDate(""); setEndDate(""); setProductName(""); setProductCode(""); setQuery({ activityNo: "", activityName: "", activityStatus: "", startDate: "", endDate: "", productName: "", productCode: "" }); switchPage(1); };
  ...
  return <PageFrame crumb="组合价" section="marketing" setView={setView}>
    <div className="filters panel activity-filter limited-discount-filter combination-price-filter"><label className="modified-time">活动时间<ActivityDateRangePicker start={startDate} end={endDate} onChange={(start, end) => { setStartDate(start); setEndDate(end); }} /></label><label>活动单号<input aria-label="活动单号" maxLength={50} value={activityNo} onChange={event => setActivityNo(event.target.value)} placeholder="请输入活动单号" /></label><label>活动名称<input aria-label="活动名称" maxLength={100} value={activityName} onChange={event => setActivityName(event.target.value)} placeholder="请输入活动名称" /></label><label>活动状态<select aria-label="活动状态" value={activityStatus} onChange={event => setActivityStatus(event.target.value)}><option value="" disabled hidden>请选择</option><option value="全部">全部</option><option value="未开始">未开始</option><option value="进行中">进行中</option><option value="已结束">已结束</option></select></label><label>商品名称<input aria-label="商品名称" maxLength={100} value={productName} onChange={event => setProductName(event.target.value)} placeholder="请输入商品名称" /></label><label>商品编码<input aria-label="商品编码" maxLength={50} value={productCode} onChange={event => setProductCode(event.target.value)} placeholder="请输入商品编码" /></label><div className="filter-actions"><button className="primary" onClick={runQuery}><FiSearch /> 查询</button><button className="secondary" onClick={reset}>重置</button></div></div>
...
function CombinationPriceForm({ setView, onSave, existingActivities = [], activity = null, mode = "create" }) {
  ...
  const save = () => {
    ...
    const { description: _description, ...activityWithoutDescription } = activity || {};
    onSave({ ...activityWithoutDescription, wareid: activity?.wareid ?? Date.now(), warecode: activity?.warecode || nextCombinationActivityNo(existingActivities), warename: title.trim(), starttime, endtime, combinationPrice: price, marketingType: "组合价", cnt: selectedProducts.length, itemList: selectedProducts.map(item => ({ ...item, wareqty: item.wareqty ?? 1 })) });
  };
