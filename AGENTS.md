...
当前 ERP 组合价原型中，所有面向用户的“组合购”文案统一使用“组合价”；不改动内部变量、接口字段或商城独立组合购域的业务术语。
...
商城组合购与 ERP 组合价完全独立运行，双方数据不创建同步、映射或回写关系。商城组合购由商城后台独立创建和维护，适用于全部门店。组合采用固定活动总价模型：不区分主商品，配置时每个组合至少包含 2 个、最多包含 9 个商品。组合价格必须大于 0 且在创建或编辑时严格低于按配置数量计算的商品原价合计。交易时若配置 SKU 因下架、停售、删除或库存不足而不可购，商城端隐藏该 SKU，组合价格保持不变，并按剩余可购商品生成订单。全局下架、停售或删除后剩余具备活动资格的 SKU 少于 2 个时，活动自动结束、释放商品促销占用且不自动恢复，原计划结束时间保持不变；仅某一门店库存不足导致少于 2 个时，只在该门店隐藏组合购，不能终止全门店活动，库存恢复且活动仍有效时可恢复展示。
